...
# Make `stype` (School type) an ordered factor variable for ordinal examples.
fct_stype <- function(x) {factor(x, levels = c("E", "M", "H"), ordered = TRUE)}

apisrs <- apisrs |> mutate(stype = fct_stype(stype))

apistrat <- apistrat |> mutate(stype = fct_stype(stype))

apiclus2 <- apiclus2 |> mutate(stype = fct_stype(stype))
```
