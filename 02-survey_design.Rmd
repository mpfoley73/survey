...
The survey design (aka, sampling strategy) depends on how study objectives are translated into survey questions, the units of analysis from the target population, the variables and covariates, and the available sampling frames for data collection [@valliant2013]. Exploratory surveys often sample individuals with qualifying characteristics by convenience, snowball (referrals), quota, or purposeful sampling [@lau2017]. In contrast to these *non-probability sampling* strategies, descriptive and explanatory surveys use *probability sampling*, including simple random sampling (SRS) and stratified sampling.^[Cluster sampling, systematic sampling and Poisson sampling are other sampling methods to at least be aware of. I'm not ready to deal with these yet.] Regardless of the survey design, sample size is set using one of three target values: i) coefficient of variation, $CV_0(\theta) = SE / \theta$; ii) $SE$; or iii) margin of error (aka, *tolerance*), $MOE = \pm z_{1-\alpha} \cdot SE$.

## Simple Random Sampling {#srs}

Suppose you have simple random sample (SRS) of size $n$ from a population of size $N$. The estimated population parameter is $\bar{Y}$ with variance $S^2$. The sample mean, $\bar{y}$, is the expected value of $\bar{Y}$. Under repeated sampling, the sample means would differ with variance
...
Equation \@ref(eq:var-of-estimate) is the square of the familiar formula for the standard error of the estimate, $SE(\bar{y}) = S / \sqrt{n} \sim s / \sqrt{n}$, multiplied by the *finite population correction* factor (FPC), $1 - n / N$. The FPC reduces the expected variance for finite (small) populations. In practice, you can ignore the FPC when $n/N < .05$. $S$ is an unknown population parameter which you estimate from the sample, $s$. A 95% CI for the population mean would be calculated as $\bar{y} \pm t_{.975,n-1} se(\bar{y})$.

The ratio of the corrected $SE$ and $\bar{Y}$ is called the *coefficient of variation*, CV. It is often expressed as its square, $CV^2(\bar{y}) = V(\bar{y}) / \bar{Y}^2$,

$$
\begin{equation}
CV^2(\bar{y}) = \left(1 - \frac{n}{N} \right) \cdot \frac{1}{n} \cdot \frac{S^2}{\bar{Y}^2}
(\#eq:cv2)
\end{equation}
$$

where $\frac{S^2}{\bar{Y}^2}$ is the square of the *unit CV* (aka *population CV*), 

$$
\begin{equation}
CV(\bar{y}_U) = \frac{S}{\bar{Y}}
(\#eq:unit-cv)
\end{equation}
$$

Solve Equation \@ref(eq:cv2) for $n = \frac{S^2 / \bar{Y}^2}{CV_0^2 + S^2 / (N \cdot \bar{Y}^2)}$ to establish the minimum sample size required to achieve a targeted CV, $CV_0(\bar{y}_s)$. Function `PracTools::nCont()` does the calculation given values for the targeted CV and unit CV. Setting the unit CV is somewhat of a chicken-and-egg problem since $S^2$ and $\bar{Y}^2$ are the population parameters you are estimating. Either rely on prior research or a best guess. The [range rule of thumb](https://www.statology.org/range-rule-of-thumb/), $S = \mathrm{range} / 4$, is a useful tool for $S^2$. The targeted CV is usually set to match or beat prior research, or is set arbitrarily. Values of 5% and 10% are common.

**Example**. Suppose from prior experience you think the unit CV is $CV(\bar{Y}) = 2$. You desire a targeted CV of $CV_0(\bar{y}) = 0.10$.
...
# Notice N was not specified. Don't worry about N if it is small. If you don't
# know CVpop or ybarU and S2, but at least have an expectation about ybarU and 
# the range, use the range rule of thumb.
expected_ybarU <- 100
expected_S <- abs(0 - 800) / 4
PracTools::nCont(CV0 = CV0, S2 = expected_S^2, ybarU = expected_ybarU) %>% ceiling()
...
If the population parameter is a proportion, $\Pi$, the CV is

$$
\begin{equation}
CV^2(\pi) = \left(1 - \frac{n}{N} \right) \cdot \frac{1}{n} \cdot \frac{N}{N-1} \cdot \frac{1 - \Pi}{\Pi}
(\#eq:unit-cv-pop)
\end{equation}
$$

where $\frac{N}{N-1} \cdot \frac{1 - \Pi}{\Pi}$ is the square of the unit CV. When $N$ is large, Equation \@ref(eq:unit-cv-pop) reduces to $CV^2(p_s) \approx \frac{1}{n} \cdot \frac{1 - \Pi}{\Pi}$. From here you can see that $n$ varies inversely with $p_U$. Function `PracTools::nProp()` calculates $n$ for proportions.
...
Whoa, $n$ was huge! You might choose to target a margin of error instead, $MOE = \pm z_{1-\alpha} \cdot SE$. Recall that $P(|\pi - \Pi| < MOE) = 1 - \alpha$. `PracTools::nPropMoe()` and `PracTools::nContMoe()` calculate $n$ for MOEs.
...
The effect of stratification relative to SRS is captured in the _design effect_ ratio, 
